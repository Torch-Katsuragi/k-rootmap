...
**問題**:
- Kotlinバージョンが古い（1.8.22）でFlutterサポートが終了予定
- Android 12以降でGPS追跡サービス開始時にクラッシュ
- `ForegroundServiceStartNotAllowedException`エラー

**解決策**:

1. **Kotlinバージョン更新**
   - `android/settings.gradle.kts`: 1.8.22 → **2.1.0**
   - Flutter最新版との互換性を確保

2. **フォアグラウンドサービスのエラーハンドリング強化**
   - 通知更新処理をtry-catchで囲み、エラー時も継続
   - サービス開始失敗時にユーザーへわかりやすいメッセージを表示
   - エラー時の適切なクリーンアップ処理

3. **Android 12以降の制限に対応**
   - 通知更新エラーを無視してGPS追跡を継続
   - サービス開始エラー時にアプリをクラッシュさせない

**修正ファイル**:
- `android/settings.gradle.kts` - Kotlin 2.1.0にアップグレード
- `lib/services/foreground_service.dart` - 通知更新のエラーハンドリング追加
- `lib/screens/map_page.dart` - サービス開始時のエラーハンドリング強化

**効果**:
- ✅ Android 12/13/14での安定動作
- ✅ GPS追跡サービスのクラッシュ防止
- ✅ Flutterの最新バージョンとの互換性維持
- ✅ ユーザーへの適切なエラーメッセージ表示

---

### 📷 カメラ撮影機能の追加 (2025-11-16)

**新機能**: アプリ内で写真を撮影し、GPS座標と撮影時刻をEXIF情報として完全に保存

**機能概要**:
- ✅ **スマホ標準カメラUI**: プレビュー、撮影、ズーム（スライダー）、カメラ切り替え
- ✅ **ズーム機能**: 縦スライダーで1.0x～最大倍率まで調整可能
- ✅ **EXIF完全対応**: GPS座標（緯度・経度・高度）、撮影日時を標準形式で記録
- ✅ **自動保存**: 現在開いているフォルダに`IMG_<タイムスタンプ>.jpg`として保存
- ✅ **PhotoNode登録**: 撮影した写真が自動的に地図上に表示される
- ✅ **権限管理**: Android/iOSのカメラ・位置情報権限に対応

**使い方**:
1. AppBarの📷カメラアイコンをタップ
2. ズームスライダーで倍率調整
3. 中央の撮影ボタンをタップ
4. ファイル名を入力（デフォルト名が入力済み、タップで全選択）
5. 決定ボタンで保存完了、自動的に地図上に表示される

**技術仕様**:
```dart
// EXIF書き込み（native_exif使用）
...
**使用パッケージ**:
- `camera: ^0.11.0+2` - Flutter公式カメラパッケージ
- `native_exif: ^0.6.2` - EXIF完全読み書き（ネイティブAPI）

**効果**:
- **現場作業の効率化**: 写真撮影とGPS記録を1タップで完了
- **柔軟なファイル名管理**: デフォルト名使用または自由に命名
- **親切なUI**: タップで全選択、素早く編集可能
- **即座に反映**: 撮影後すぐに地図上に表示
- **完全削除対応**: layer drawerから削除すると画像ファイルも削除
- **EXIF標準準拠**: QGISやGoogle Earthで位置情報を完全に読み取れる
- **データの一元管理**: 写真とGeoPackageデータを同じフォルダで管理
- **使いやすさ**: スマホカメラと同じ感覚で操作可能

**プラットフォーム対応**:
- ✅ Android - 完全対応
- ✅ iOS - 完全対応
- ❌ Windows/Web - カメラアイコン非表示（非対応）

---

### QGIS標準形式への移行（PRIMARY KEY: id → fid）+ フリーズ・クラッシュ対策 (2025-11-16)
