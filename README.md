...
- **機能詳細**:
  - 1秒間隔でのGPS情報付きログ出力（デバッグ・テスト・追跡用） **【NEW】**
  - リアルタイムGPS座標取得（緯度・経度・精度情報） **【NEW】**
  - Android端末では通知バーでサービス状態とGPS座標を表示
  - UIからのサービス開始・停止制御
  - GPS権限・サービス状態の事前確認 **【NEW】**
  - エラーハンドリングとログ出力の最適化
...
2. プロジェクトフォルダを選択
3. 自動的にマップ画面に遷移
4. GPS追跡サービスは地図画面のGPS情報バーから制御可能 **【NEW】**
5. デバッグコンソールで1秒間隔のGPS情報ログ確認 **【NEW】**
6. Android端末では通知バーでリアルタイムGPS座標確認 **【NEW】**
