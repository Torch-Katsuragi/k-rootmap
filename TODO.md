...
- [/] **View の導入** — Layer の下に「フィルタ＋スタイルの集合体」を置く。QGISのレイヤと1:1対応
  ...
  - [ ] **段4b: View ごとのスタイルが描画に出ない**
        地図は全レイヤのフィーチャを共有ソースに流し込み、全体で1組のスタイルで描いている。
        View 以前からの制約で、レイヤ単位のスタイル設定UIも同じ理由で効いていない。
        やるならフィーチャに所属Viewを持たせ、Viewごとにフィルタ付きスタイルレイヤを足す
  - [ ] View の z順（同一レイヤ内の並び）も描画には未反映。並びの保存だけしてある
- [/] **`.qgs` ライター** — dir/gpkg/layer をレイヤグループ、View をレイヤとして出力
  - [x] 実装（`lib/services/qgis/`）。書き出し導線は 地図の ≡ メニュー と
        フォルダのメニュー（共有の単位が dir なので両方から）
  - [ ] ⚠ **QGISで実際に開けるかは未検証。** 開発機にQGISが入っていない。
        テストで見ているのはXMLの形まで（[[docs/technical/qgis-interop]]）
