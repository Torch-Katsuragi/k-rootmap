...
**View ごとにスタイルを変えても描画に出ない。** `KMetaLayerStyle` は View に
持てるし保存もされるが、地図は全レイヤのフィーチャを共有ソース
（`k-polygons` / `k-lines` / `k-points`）に流し込み、**全体で1組のスタイル**で
描いている。これは View 以前からの制約で、レイヤ単位のスタイル設定UIも
同じ理由で描画に効いていない。

やるなら「フィーチャに所属Viewを持たせ、View ごとにフィルタ付きスタイルレイヤを
足す」形になる。ソースは共有のままでよいのでデータ転送は増えない。
`.qgs` ライター（段5）は View のスタイルをそのまま書けばよいので、
段4b を待たずに進められる。
